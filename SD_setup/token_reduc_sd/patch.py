...
def compute_merge(x: torch.Tensor, tome_info: Dict[str, Any]) -> Tuple[Callable, ...]:
    ...
    if downsample <= args["max_downsample"]:
        ...
        m, u = merge_fn(x, w, h, args["sx"], args["sy"], r, linkage=linkage, no_rand=not use_rand, generator=args["generator"])
    else:
...
def apply_patch(
    ...
    merge_fn_str = merge_fn_str.lower()
    if merge_fn_str == "tome":
        merge_fn = merge.bipartite_soft_matching_random2d
    elif merge_fn_str == "atc":
        merge_fn = merge.agglomerative_clustering
    else:
        raise RuntimeError(f"Provided invalid merge function string: {merge_fn_str}")
