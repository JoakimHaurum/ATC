...
def agglomerative_clustering(
    ...
    with torch.no_grad():
        ...
        cluster_labels = np.zeros((B,N),dtype=np.int64)
        for b_idx in range(B):
            labels = clustering.fit(scores[b_idx]).labels_
            #Z = linkage_fns(scores[b_idx][upper_traingle_indexes], method = linkage)
            #labels = fcluster(Z, t=Z[N-num_clusters-1, 2], criterion="distance") - 1
            cluster_labels[b_idx] = labels
        
        cluster_labels = torch.from_numpy(cluster_labels).to(device = metric.device)
   
    def merge(x: torch.Tensor, mode="mean") -> torch.Tensor:
        dst = torch.zeros(B, num_clusters, C, device=x.device, dtype=x.dtype)
        dst = dst.scatter_reduce(-2, cluster_labels.unsqueeze(-1).repeat(1,1,C), x, reduce=mode, include_self=False)
        return dst
    ...
    def unmerge(x: torch.Tensor) -> torch.Tensor:
        ...
        first_half_out = gather(x, dim=-2, index=first_index)
        second_half_out = gather(x, dim=-2, index=second_index)
