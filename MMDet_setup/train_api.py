...
def train_detector(model,
    ...
    gc_cfg = cfg.get('cumulative_iters', 1)
    if fp16_cfg is None and cfg.get('device', None) == 'npu':
        fp16_cfg = dict(loss_scale='dynamic')
    if fp16_cfg is not None:
        if gc_cfg > 1:
            optimizer_config = GradientCumulativeFp16OptimizerHook(
                **cfg.optimizer_config, **fp16_cfg, cumulative_iters=gc_cfg, distributed=distributed)
        else:
            optimizer_config = Fp16OptimizerHook(
                **cfg.optimizer_config, **fp16_cfg, distributed=distributed)
    elif distributed and 'type' not in cfg.optimizer_config:
        if gc_cfg > 1:
            optimizer_config = GradientCumulativeOptimizerHook(**cfg.optimizer_config, cumulative_iters = gc_cfg)
        else:
