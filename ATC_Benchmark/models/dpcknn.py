...
class CTM(nn.Module):
    ...
    def forward(self, x, idx_token, agg_weight):
...
class DPCKNNVisionTransformer(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.cluster_count = args.reduction_ratio
        self.k_neighbors = args.k_neighbors
        self.equal_weight = args.equal_weight

        if len(self.cluster_count) == 1:
            self.cluster_count = [int(self.patch_embed.num_patches * (args.reduction_ratio[0] ** (idx+1))) for idx in range(len(self.cluster_loc))]
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.cluster_loc:
                ...
                x, idx_token, agg_weight = self.cluster_layers[cluster_cnt](x[:,self.num_prefix_tokens:], idx_token, agg_weight)
