    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.cluster_loc:
                ...
                token_weights = torch.sum(torch.sum(attn, dim=1), dim = 1)[:, self.num_prefix_tokens:].unsqueeze(2)
                x, _, _ = self.cluster_layers[cluster_cnt](x[:,self.num_prefix_tokens:], token_weights)  
